// Super Admin: hard-delete all student accounts platform-wide.
// Removes student-owned data, profile, role mapping, and the auth user.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!roleRow) return json(403, { error: 'Super admin access required' })
    ...
    if (!ids.length) return json(200, { deleted: 0, message: 'No student accounts to delete' })

    // Wipe student-owned rows. Order chosen to avoid FK conflicts.
    const studentTables = [
      'attendance',
    ...
    console.error('reset-students error', e)
